      ...
      <el-table-column label="分类" width="110px" align="center">
        <template slot-scope="scope">
          <span class="link-type" @click="showDialog(scope.row.id)"
            >{{ scope.row.columnTitle }} <i class="el-icon el-icon-edit"></i
          ></span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
      ...
      if (this.currentEditArticleId != null) {
        article
          ...
          .then((res) => {
            callback(false);
            if (res.success) {
              let currentArticle=this.list.filter(n=>n.id==this.currentEditArticleId)[0]
    ...
    getList() {
      ...
      article.getPage(this.listQuery).then((response) => {
    ...
    handleMultiDel() {
      ...
      article.batchDelete(this.sels.join(',')).then(res=>{
        res.success? this.$mtip.success(res.message):this.$mtip.error(res.message)
         this.getList()
        loading.close()
      }).catch(error=>{
          loading.close()
    ...
    showDialog(id) {
      ...
      this.currentChooseCat = this.list.filter((n) => n.id == id)[0].columnId;
      let loading = this.$mtip.loading();
      GetAllColum()
        .then((res) => {
          this.columnList = res;
    ...
    editCat() {
      if (this.currentChooseArticleId != null) {
        let loading = this.$mtip.loading();
        article
          .updataColumn(this.currentChooseArticleId, this.currentChooseCat)
          .then((res) => {
            if(res.success){
               let currentArticle=this.list.filter(n=>n.id==this.currentChooseArticleId)[0]
               let curCat=this.columnList.filter(n=>n.id==this.currentChooseCat)[0]
               currentArticle.columnId=curCat.id
               currentArticle.columnTitle=curCat.title
              this.$mtip.success(res.message)
            }else{
              this.$mtip.error(res.message);
            }
